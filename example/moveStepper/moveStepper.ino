...
void loop() {
  ...
  if (state == 0) { // State 0: Reset the tray to the 0 position
    if (myTray.resetBack() == true) {
      state++;
    }
  }
  else if (state == 1) {  // State 1: Check for Serial Monitor input
  ...
  else if (state == 2) {  // State 2: Move the tray to the target position
